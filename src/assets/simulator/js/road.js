...
function road(
	...
	this.isGame =
		scenarioString === "RampMeteringGame" ||
...
road.prototype.getAttributes = function (fracTruck, fracOthers) {
	...
	var others_length =
		typeof scooter_length === "undefined" ? 2.5 : scooter_length;
	var others_width =
		typeof scooter_width === "undefined" ? 1.2 : scooter_width;
	var r = Math.random();
	var vehType =
		r < fracTruck
			? "truck"
			: r < fracTruck + others_frac
			? "others"
			: "car";
	var lengthOut =
		vehType === "car"
	...
	var widthOut =
		vehType === "car"
			...
			? truck_width
			: others_width;
	var attributes = { type: vehType, len: lengthOut, width: widthOut };
	if (false) {
		console.log(
			"road.getAttributes: r=",
			r,
			" fracTruck=",
			fracTruck,
			" fracTruck+others_frac=",
			fracTruck + others_frac
		);
	}
	return attributes;
...
road.prototype.initRegularVehicles = function (
	densityPerLane,
	fracTruck,
	fracScooter,
	speedInit
	...
	for (var i = 0; i < nvehPlus; i++) {
		...
		var fracTruck =
			lane === this.nLanes - 1 ? fracTruckRight : fracTruckRest;
		...
		if (this.veh[i].u >= uminLoc && this.veh[i].u <= umaxLoc) {
			console.log(
				" veh[" +
					...
					this.veh[i].len +
					"  u=" +
					parseFloat(this.veh[i].u, 10).toFixed(1) +
					"  lane=" +
					this.veh[i].lane +
					"  v=" +
					parseFloat(this.veh[i].v, 10).toFixed(1) +
					"  speed=" +
					parseFloat(this.veh[i].speed, 10).toFixed(1) +
					"  acc=" +
					parseFloat(this.veh[i].acc, 10).toFixed(1) +
					"  iLead=" +
					this.veh[i].iLead +
					"  iLag=" +
					this.veh[i].iLag +
					"  iLeadRight=" +
					this.veh[i].iLeadRight +
					"  iLagRight=" +
					this.veh[i].iLagRight +
					"  iLeadLeft=" +
					this.veh[i].iLeadLeft +
					"  iLagLeft=" +
					this.veh[i].iLagLeft +
					""
			);
...
road.prototype.writeVehiclesSimple = function (umin, umax) {
	console.log(
		"\n\nIn road.writeVehiclesSimple(): roadID=",
		this.roadID,
		" nveh=",
		this.veh.length,
		" nLanes=",
		...
		if (this.veh[i].u >= uminLoc && this.veh[i].u <= umaxLoc) {
			console.log(
				" veh[" +
					...
					this.veh[i].lane +
					"  dvdt=" +
					parseFloat(this.veh[i].dvdt, 10).toFixed(2) +
					"  speed=" +
					parseFloat(this.veh[i].speed, 10).toFixed(1) +
					"  acc=" +
					parseFloat(this.veh[i].acc, 10).toFixed(1) +
...
road.prototype.writeVehiclesSimpleToFile = function (filename) {
	console.log(
		"\nin road.writeVehiclesSimpleToFile(): roadID=",
		this.roadID,
		" filename=",
		filename
	);
...
road.prototype.updateExportString = function () {
	...
	if (rest < dt - 0.0001) {
		for (var i = 0; i < this.veh.length; i++) {
			var heading =
				this.veh[i].speed > 1e-4
					? this.veh[i].dvdt / this.veh[i].speed
					: 0;
			this.exportString =
				this.exportString +
				"\n" +
...
road.prototype.writeSpeedlimits = function (umin, umax) {
	console.log(
		"\nin road.writeSpeedlimits(): roadID=",
		this.roadID,
		" nveh=",
		this.veh.length,
		" nLanes=",
			...
			if (this.veh[i].isRegularVeh()) {
				console.log(
					" veh[" +
						...
						" speed=" +
						formd(this.veh[i].speed) +
						" v0=" +
						formd(this.veh[i].longModel.v0) +
						" acc=" +
						parseFloat(this.veh[i].acc, 10).toFixed(1) +
						""
				);
	...
	for (var i = 0; i < this.veh.length; i++) {
		if (this.veh[i].id >= idmin && this.veh[i].id <= idmax) {
			var s =
				i == 0
					? 10000
					: this.veh[i - 1].u - this.veh[i - 1].len - this.veh[i].u;
			console.log(
				"t=",
				parseFloat(dt * itime).toFixed(1),
				" veh",
				i,
				" type=",
				this.veh[i].type,
				...
				this.roadID,
				" u=",
				parseFloat(this.veh[i].u, 10).toFixed(3),
				//" v=",parseFloat(this.veh[i].v,10).toFixed(1),
				//" lane=",this.veh[i].lane,
				//" dvdt=",parseFloat(this.veh[i].dvdt,10).toFixed(2),
				" s=",
				parseFloat(s).toFixed(3),
				" speed=",
				parseFloat(this.veh[i].speed, 10).toFixed(3),
				" acc=",
				parseFloat(this.veh[i].acc, 10).toFixed(3),
				""
			);
		}
	}
...
road.prototype.writeVehicleRoutes = function (umin, umax) {
	console.log(
		"\nin road.writeVehicleRoutes: roadID=",
		this.roadID,
		" length=",
		parseFloat(this.roadLen).toFixed(1),
		" nveh=",
		this.veh.length,
		" itime=",
		itime,
...
road.prototype.writeDepotVehObjects = function (umin, umax) {
	console.log(
		"itime=",
		itime,
		"in road.writeDepotVehObjects: roadID=",
		this.roadID,
		" nveh=",
		this.veh.length
	);
...
road.prototype.writeVehicleLongModels = function (umin, umax) {
	console.log(
		"\nin road.writeVehicleLongModels(): roadID=",
		this.roadID,
		" nveh=",
		this.veh.length,
		" itime=" + itime
		...
		if (this.veh[i].u >= uminLoc && this.veh[i].u <= umaxLoc) {
			...
			console.log(
				" veh[" +
					...
					parseFloat(this.veh[i].speed, 10).toFixed(3) +
					"  v0=" +
					parseFloat(this.veh[i].longModel.v0).toFixed(3) +
...
road.prototype.writeVehicleLCModels = function () {
	console.log(
		"\nin road.writeVehicleLCModels(): roadID=",
		this.roadID,
		" nveh=",
		this.veh.length,
		" itime=" + itime
...
road.prototype.getNearestUof = function (otherRoad, u) {
	...
	for (var i = 0; i <= otherRoad.nSegm; i++) {
		...
		var dist2 =
			Math.pow(xOwn - otherRoad.xtab[i], 2) +
...
road.prototype.findNearestVehTo = function (xUser, yUser, filterFun) {
	...
	for (var i = 0; i < this.veh.length; i++) {
		var filterPassed =
			typeof filterFun === "undefined" ? true : filterFun(this.veh[i]);
		...
		if (filterPassed) {
			var u = this.veh[i].u;
			var dist2 =
				Math.pow(xUser - this.traj[0](u), 2) +
...
road.prototype.findNearestDistanceTo = function (xUser, yUser) {
	...
	var phiNorm = this.get_phi(uReturn, this.traj) - 0.5 * Math.PI; // angle in v direction
	var sign_v =
		Math.cos(phiNorm) * dxReturn + Math.sin(phiNorm) * dyReturn > 0
			? 1
			: -1;
	...
	if (false) {
		console.log(
			"end road.findNearestDistanceTo:",
			" roadID=",
			this.roadID,
			" xUser=",
			xUser,
...
road.prototype.findFollowerAt = function (u) {
	...
	if (vehFollow.id == -1) {
		console.log(
			"road.findFollowersAt: warning: no follower at position ",
			u,
			" on any lane"
		);
	}
...
road.prototype.testForNaN = function () {
	// detect the fucking NaN's
	for (var i = 0; i < this.nSegm; i++) {
...
road.prototype.initializeMicro = function (
	types,
	lengths,
	widths,
	longPos,
	...
	if (
		lengths.length != nvehInit ||
		widths.length != nvehInit ||
		longPos.length != nvehInit ||
		lanesReal.length != nvehInit ||
	...
	if (this.veh.length > 0) {
		this.veh.splice(0, this.veh.length);
	}
	...
	for (var i = 0; i < types.length; i++) {
		...
		var type =
			types[i] === 0 ? "car" : types[i] === 1 ? "truck" : "obstacle";
		var lane = Math.round(lanesReal[i]);
		var vehNew = new vehicle(
			lengths[i],
			widths[i],
			longPos[i],
			lane,
	...
	if (false) {
		console.log(
			"road.initializeMicro: initialized with ",
			this.veh.length,
			" vehicles"
		);
...
road.prototype.updateTruckFrac = function (fracTruck, mismatchTolerated) {
	if (this.veh.length > 0) {
		...
		if (Math.abs(fracTruckReal - fracTruck) > mismatchTolerated) {
			...
			var diffSpace =
				(fracTruckTooLow ? -1 : 1) * (truck_length - car_length);
			...
			if (fracTruckTooLow) {
				// change cars->trucks on the right lane if possible
				var maxSpace = 0;
				for (var lane = this.nLanes - 1; lane >= 0; lane--) {
					if (!success) {
						for (var i = 0; i < nActual; i++) {
							if (
								this.veh[i].lane === lane &&
								this.veh[i].type === candidateType
							) {
								...
								if (iLag < i) {
									s += this.roadLen;
								} //periodic BC (OK for open BC as well)
								if (s > maxSpace) {
									k = i;
									maxSpace = s;
								}
				...
				// change trucks->cars: transform truck with smallest space
				var minSpace = 10000;
				for (var i = 0; i < nActual; i++) {
					if (this.veh[i].type === candidateType) {
						...
						var iLag = this.veh[i].iLag;
						var s =
							this.veh[i].u - this.veh[iLag].u - this.veh[i].len;
						if (iLag < i && s < 0) {
							s += this.roadLen;
						} //periodic BC (OK for open BC as well)
						if (s < minSpace) {
...
road.prototype.updateDensity = function (density) {
	...
	if (nActual > nDesired) {
		// too many vehicles, remove one per time step
		var r = Math.random();
	...
	// too few vehicles, generate one per time step in largest gap
		...
		if (success) {
			// otherwise, no veh added
			if (!emptyLanes) {
				uNew = this.veh[k].u + 0.5 * (car_length + maxSpace);
				if (uNew > this.roadLen) {
					uNew -= this.roadLen;
				} //periodic BC
				laneNew = this.veh[k].lane;
				speedNew =
					0.5 *
					(this.veh[k].speed + this.veh[this.veh[k].iLead].speed);
			}
			...
			if (emptyLanes) {
				vehNew.speed = longModelTruck.v0;
			}
...
road.prototype.initMergeDiverge = function (
	targetRoads,
	isMerge,
	mergeDivergeLen,
	uLast,
	...
	for (var ir = 0; ir < targetRoads.length; ir++) {
		...
		if (false) {
			console.log(
				"\nin road.initMergeDiverge: this.mergeDivergeInfo[" +
					ir +
					"]=",
				this.mergeDivergeInfo[ir]
			);
...
road.prototype.initConnect = function (
	targetRoads,
	uSource,
	offsetLane,
	LCbias
...
road.prototype.setCFModelsInRange = function (
	umin,
	umax,
	longModelCar,
	longModelTruck
	...
	for (var i = 0; i < this.veh.length; i++) {
		...
		if (u > umin && u < umax) {
			...
			if (this.veh[i].type === "car") {
				this.veh[i].longModel.copy(longModelCar);
			}
			if (this.veh[i].type === "others") {
				this.veh[i].longModel.copy(longModelCar);
			}
			if (this.veh[i].type === "truck") {
...
road.prototype.setLCModelsInRange = function (
	umin,
	umax,
	LCModelCar,
	LCModelTruck,
	...
	for (var i = 0; i < this.veh.length; i++)
		if (this.veh[i].isRegularVeh()) {
			...
			if (u > umin && u < umax) {
				...
				if (toRight) {
					this.veh[i].LCModel.copy(this.LCModelMandatoryRight);
				} else {
					this.veh[i].LCModel.copy(this.LCModelMandatoryLeft);
				}
			}
...
road.prototype.update_iLeadRight = function (i) {
	...
	if (this.veh[i].lane < this.nLanes - 1) {
		...
		success =
			i === iLeadRight ||
			this.veh[iLeadRight].lane === this.veh[i].lane + 1;
		while (!success) {
...
road.prototype.update_iLagRight = function (i) {
	...
	if (this.veh[i].lane < this.nLanes - 1) {
		...
		success =
			i === iLagRight ||
			this.veh[iLagRight].lane === this.veh[i].lane + 1;
		while (!success) {
...
road.prototype.update_iLeadLeft = function (i) {
	...
	if (this.veh[i].lane > 0) {
		...
		success =
			i === iLeadLeft ||
			this.veh[iLeadLeft].lane === this.veh[i].lane - 1;
		while (!success) {
...
road.prototype.update_iLagLeft = function (i) {
	...
	if (this.veh[i].lane > 0) {
		...
		success =
			i === iLagLeft || this.veh[iLagLeft].lane === this.veh[i].lane - 1;
		while (!success) {
			iLagLeft = iLagLeft === n - 1 ? 0 : iLagLeft + 1;
			success =
				i === iLagLeft ||
				this.veh[iLagLeft].lane === this.veh[i].lane - 1;
		}
	} else {
		iLagLeft = -10;
	}
...
road.prototype.calcAccelerations = function () {
	...
	for (var i = 0; i < this.veh.length; i++) {
		...
		if (iLead === -100) {
			console.log(
				"road.calcAccelerations: i=",
				i,
				" iLead=",
				iLead,
				" should not happen!! possibly this.updateEnvironment() missing!"
			);
		}
		var s = this.veh[iLead].u - this.veh[iLead].len - this.veh[i].u;
		...
		if (iLead >= i) {
			// vehicle i is leader, for any BC iLead defined
			if (this.isRing) {
				s += this.roadLen;
			} // periodic BC; accLead OK
			else {
				s = 10000;
				accLead = 0;
			} // free outflow BC: virt veh 10km away
		}
		...
		if (false) {
			...
			var veh = this.veh[i];
			console.log(
				"after calcAccelerations: veh id=",
				veh.id,
				"leader id=",
				this.veh[iLead].id,
				"u=" + veh.u.toFixed(1),
...
road.prototype.updateEgoVeh = function (externalEgoVeh) {
	...
	if (iEgo === -1) {
		console.log(
			"warning: road.updateEgoVeh called" +
	...
	var dotxRoadAxis =
		(0.5 * Math.PI - this.get_phi(ego.u, this.traj)) * ego.speed;
	...
	var acc_v =
		(externalEgoVeh.aLat + roadCurv * ego.speed * ego.speed) /
	...
	if (externalEgoVeh.latCtrlModel === 2) {
		...
		// zero point at right lane = starting lane v=(nLanes-1)
		ego.v =
			this.nLanes - 1 + (externalEgoVeh.v - xRoadAxis) / this.laneWidth;
		ego.dvdt = (externalEgoVeh.vv - dotxRoadAxis) / this.laneWidth;
		//ego.dvdt=(externalEgoVeh.vv-0)/this.laneWidth;
		if (false) {
			console.log(
				"road.updateEgoVeh: latCtrlModel=",
	...
	if (false) {
		console.log(
			"in road.updateEgoVeh:",
			" time=",
			parseFloat(time).toFixed(2),
			" dt=",
			parseFloat(dt).toFixed(2),
			" laneWidth=",
			parseFloat(this.laneWidth).toFixed(1),
			" acc=",
			parseFloat(ego.acc).toFixed(2),
			" acc_v[m/s^2]=",
			parseFloat(acc_v).toFixed(2),
			"\n         u=",
			parseFloat(ego.u).toFixed(2),
			" v[laneWidth]=",
			parseFloat(ego.v).toFixed(2),
			" lane=",
			ego.lane,
			"\n         speedLong=",
			parseFloat(ego.speed).toFixed(2),
			" vv=",
			parseFloat(vv).toFixed(2),
			" dvdt=",
			parseFloat(ego.dvdt).toFixed(2)
		);
		//this.writeVehiclesSimple();
	}
}; // updateEgoVeh
	...
	for (var i = 0; i < this.veh.length; i++) {
		if (this.veh[i].id != 1 && this.veh[i].isRegularVeh()) {
			...
			this.veh[i].u += Math.max(
				0,
				this.veh[i].speed * dt + 0.5 * this.veh[i].acc * dt * dt
			);

			// longitudinal speed update

			this.veh[i].speed = Math.max(
...
road.prototype.doChangesInDirection = function (toRight) {
	...
	for (var i = 0; i < this.veh.length; i++) {
		...
		var outsideLCban =
			this.veh[i].u < this.LCbanStart || this.veh[i].u > this.LCbanEnd;
		...
		if (false) {
			console.log(
				...
				this.veh[i].u.toFixed(2),
				" roadLen=",
				this.roadLen.toFixed(2),
				" LCbanStart=",
				this.LCbanStart,
				" LCbanEnd=",
				this.LCbanEnd,
				" beyondStart=",
				beyondStart ? "true" : "false",
				" outsideLCban=",
				outsideLCban ? "true" : "false",
				" regularVeh=",
				this.veh[i].isRegularVeh() ? "true" : "false",
				""
			);
		}
		...
		if (
			...
			var lastChangeSufficTimeAgo =
				this.veh[i].dt_afterLC > this.waitTime &&
				this.veh[i].dt_lastPassiveLC > 0.2 * this.waitTime;

			if (false) {
				console.log(
					"road.doChangesInDirection: vehID= ",
					this.veh[i].id,
					" time=",
					time,
					" i=",
			...
			if (targetLaneExists && lastChangeSufficTimeAgo) {
				...
				if (
					typeof this.veh[iLead] === "undefined" ||
					...
					iLagNew = toRight
						...
						: this.veh[i].iLagLeft;
					console.log(
						"in road.changeLanes: something went wrong, needed to reorder calling this.updateEnvironment(); check if forgot somewhere..."
					);
				}
				...
				if (
					...
					var sNew =
						this.veh[iLeadNew].u -
						this.veh[iLeadNew].len -
						this.veh[i].u;
					var sLagNew =
						this.veh[i].u - this.veh[i].len - this.veh[iLagNew].u;
					...
					if (iLeadNew >= i) {
						// if iLeadNew=i => laneNew is empty
						if (this.isRing) {
							sNew += this.roadLen;
						} // periodic BC
						else {
					...
					if (this.veh[iLagNew].longModel === undefined) {
						console.log(
							"vehicle ",
							iLagNew,
							" has no longModel! vehicle=",
							this.veh[iLagNew]
						);
						this.writeNonregularVehicles(0, this.roadLen);
					}
					var accLagNew = this.veh[iLagNew].longModel.calcAccDet(
						sLagNew,
						speedLagNew,
						speed,
						accNew
					...
					if (false) {
						...
						console.log(
							...
							toRight,
							" bThr=",
							this.veh[i].LCModel.bThr.toFixed(3),
							" bBiasRight=",
							this.veh[i].LCModel.bBiasRight.toFixed(3),
							" MOBILOK=",
							MOBILOK
						);
						...
						if (false) {
							...
							var speed = this.veh[i].speed;
							var speedLead = this.veh[iLead].speed;
							var accCalc = this.veh[i].longModel.calcAccDet(
								// within MOBIL
								s,
								speed,
								speedLead,
							...
							console.log(
								...
								this.veh[iLead].id,
								" leadNew:",
								this.veh[iLeadNew].id,
								" lagNew:",
								this.veh[iLagNew].id,
								"\n  u=",
								parseFloat(this.veh[i].u).toFixed(3),
								"  s=",
								parseFloat(s).toFixed(3),
								" speed=",
								parseFloat(speed).toFixed(3),
								" speedLead=",
								parseFloat(speedLead).toFixed(3),
								" accLead=",
								parseFloat(accLead).toFixed(3),
								" acc=",
								parseFloat(this.veh[i].acc).toFixed(3),
								" accNew=",
								parseFloat(accNew).toFixed(3),
				...
				if (
					scenarioString == "OnRamp_BaWue" ||
					scenarioString == "roadworks_BaWue"
				) {
					if (
						changeSuccessful &&
						this.veh[i].type === "truck" &&
						!toRight
					) {
						console.log(
							"road.doChangesInDirection(): preventing truck by force to change to the left, check why this happens:\n vehicle=",
							this.veh[i]
						);
...
road.prototype.connect = function (
	targetRoad,
	uSource,
	uTarget,
	offsetLane,
	...
	var targetHasPriority =
		typeof targetPrio === "undefined" ? false : targetPrio;
	...
	for (var iveh = 0; iveh < this.veh.length; iveh++) {
		...
		var inInfluenceRegion =
			this.veh[iveh].isRegularVeh() &&
		...
		if (inInfluenceRegion && routeOK) {
			...
			var laneContinues =
				lane + offsetLane >= 0 && lane + offsetLane < targetRoad.nLanes;
			var gluedTogether =
				laneContinues && !potentialConflictsExist && uTarget < 0.0001;
			...
			var accFree = vehConnect.longModel.calcAccDet(
				100000,
				speed,
				speed,
				0
			...
			if (this.connectLog) {
				var zoneStr = C1
					? " in braking zone"
					: C2
					? " in reversible decision zone"
					: C3
					? " in final decision zone"
					: " about to transfer";
				console.log(
			...
			if (laneContinues && noOwnLeader) {
				...
				var targetFlwInfo = targetRoad.findFollowerAtLane(
					uTargetInst,
					lane + offsetLane
				);
				var targetFlwExists = // no follower complication if road not left!
					targetFlwInfo[0] &&
					this.roadID != targetRoad.roadID &&
					!(targetRoad.veh[targetFlwInfo[1]].type == "obstacle");

				var iTargetFlw = targetFlwInfo[1];
				var uTargetFlw = targetFlwExists
					? targetRoad.veh[iTargetFlw].u
					: -1000000;
				var speedTargetFlw = targetFlwExists
					? targetRoad.veh[iTargetFlw].speed
					: 0;
				var sTargetFlw = targetFlwExists
					? uTargetInst - vehConnect.len - uTargetFlw
					: 1000000;

				var TargetLdrInfo = targetRoad.findLeaderAtLane(
					uTargetInst,
					lane + offsetLane
				);
				...
				var speedTargetLdr = TargetLdrExists
					? targetRoad.veh[iTargetLdr].speed
					: 0;
				var uTargetLdr = TargetLdrExists
					? targetRoad.veh[iTargetLdr].u
					: +1000000;
				var lenTargetLdr = TargetLdrExists
					? targetRoad.veh[iTargetLdr].len
					: 0;
				var sTargetLdr = TargetLdrExists
					? uTargetLdr - lenTargetLdr - uTargetInst
					: 100000;

				if (this.connectLog) {
					console.log(
						"   connect (3): through lane,",
						"acc from calcAccelerations=",
						vehConnect.acc.toFixed(2),
						"\n   connect (3a): TargetLdrID=",
						TargetLdrExists
							? targetRoad.veh[iTargetLdr].id
							: "none",
						"sTargetLdr=",
						sTargetLdr.toFixed(1),
						" followerID=",
						targetFlwExists
							? targetRoad.veh[iTargetFlw].id
							: "none",
						"sTargetFlw=",
						sTargetFlw.toFixed(1),
						""
					);
				}
				...
				if (gluedTogether) {
					if (TargetLdrExists) {
						// overwrite accConnect (preset with accFree)
				...
				if ((C2 || C3) && !gluedTogether) {
					...
					if (targetFlwExists && speed < speedTargetFlw) {
						...
						var anticipatedTargetFlwTheSame =
							uTargetFlw + speedTargetFlw * tc < uTarget;
						var newAnticipatedTargetFlw =
							!anticipatedTargetFlwTheSame;
						...
						while (newAnticipatedTargetFlw && iLag > iTargetFlw) {
							...
							newAnticipatedTargetFlw =
								uTargetFlw + speedTargetFlw * tc > uTarget;
							iLag = targetRoad.veh[iTargetFlw].iLag;
						}

						if (!anticipatedTargetFlwTheSame) {
							// otherwise use initialisation
							sTargetFlw =
								uTarget -
								uTargetFlw -
								(uSource - u) -
					...
					if (targetFlwExists) {
						// then also in C2||C3, exclusive to (3b),(3c)
						accTargetFlw = targetRoad.veh[
							iTargetFlw
						].longModel.calcAccDet(
							sTargetFlw,
							speedTargetFlw,
							speed,
							accFree
						...
						var IDMbTargetFlw =
							targetRoad.veh[iTargetFlw].longModel.b;

						if (targetHasPriority) {
							decideGo = accTargetFlw >= -bPrio;
							revertGo =
								C2 &&
								accTargetFlw < -IDMbTargetFlw &&
								accStop > Math.min(-bSafe, accTargetFlw);
						} else {
							// own has prio but target cannot see => respect also here
					...
					if (TargetLdrExists) {
						// here for !gluedTogether, slight double code
						accConnect = vehConnect.longModel.calcAccDet(
							// overwrite accConnect
							sTargetLdr,
							speed,
							speedTargetLdr,
							0
						...
						decideGo =
							decideGo && accConnect > -vehConnect.longModel.b;
					...
					vehConnect.finalGo =
						vehConnect.finalGo ||
						(C3 &&
							vehConnect.tookGoDecision &&
							!vehConnect.conflictsExist);

					if (C2 && vehConnect.conflictsExist) {
						// C2: revert positive decision
						vehConnect.tookGoDecision = false;
					}
 	...
 	###############################################################*/
				...
				if (gluedTogether) {
					vehConnect.acc = accConnect;
				}
				if (!gluedTogether) {
					if (C1) {
						vehConnect.acc = accStop;
					} else {
						//else if(C2||C3){
				...
				if (targetFlwExists && !targetHasPriority && (C2 || C3)) {
					...
					console.log(
						"(6b): accTargetFlw=" + accTargetFlw.toFixed(1)
					);
				}

				if (this.connectLog) {
					console.log(
						"   connect (6): final acceleration:",
			...
			if (u > uSource && this.roadID != targetRoad.roadID) {
				if (this.connectLog) {
					...
					console.log(
						"  connect (7): All previous actions drove the vehicle",
			...
			if (this.connectLog) {
				console.log(
					"   exiting road.connect: itime=" + itime,
...
road.prototype.determineConflicts = function (
	vehConnect,
	uSource,
	uTarget,
	conflicts,
	...
	if (log) {
		console.log(
			"\n=====================================================" +
				"\nroad.determineConflicts for veh " +
				vehConnect.id,
			":" + "t=" + time.toFixed(1)
		);
	}
	...
	for (var ic = 0; ic < conflicts.length && noConflictDetected; ic++) {
		...
		var tc =
			-speed / acc +
			Math.sqrt(Math.pow(speed / acc, 2) + (2 * duOwn) / acc);
		...
		if (log) {
			console.log(
				"  check conflict" + ic,
				"caused by road " + conflicts[ic].roadConflict.roadID,
				":" + "uSource-u=" + (uSource - u).toFixed(1),
		...
		for (var iveh = 0; goOnCrit; iveh++) {
			...
			if (vehConflict.isRegularVeh()) {
				...
				// only some conflicting road destinations conflict
					...
					var indexOrigin = vehConflict.route.indexOf(
					...
					var conflictDestID =
						vehConflict.route.length == indexOrigin + 1
					...
					if (log) {
						console.log(
							"  determineConflicts, checking candidates:" +
								"vehConflict.id=" +
								vehConflict.id,
			...
			if (isCandidate) {
				...
				var ducOtherVeh =
					ucOther - (vehConflict.u + speedConflict * tc);
				...
				noConflictDetected =
					Math.abs(xtc) > XTC && (ttc > TTCup || ttc < -TTCdown);
				...
				if (log) {
					...
					var xConflictOther =
						conflicts[ic].roadConflict.traj[0](ucOther);
					var yConflictOther =
						conflicts[ic].roadConflict.traj[1](ucOther);
					var dist2 =
						Math.pow(xConflictOwn - xConflictOther, 2) +
					...
					if (dist2 >= 16) {
						console.log(
							...
							"OD " + this.roadID,
							targetRoad.roadID,
							"conflicts with road ID=" +
								conflicts[ic].roadConflict.roadID,
							"veh " + vehConflict.id,
							"having route=" + vehConflict.route,
							...
							"ducOtherVeh(now)=" +
								(ucOther - vehConflict.u).toFixed(1),
	...
	if (log) {
		console.log(
			"Leaving determineConflicts, conflictsExist=" + conflictsExist,
			"\n=============================================\n\n"
		);
	}
...
road.prototype.mergeDiverge = function (
	otherRoad,
	offset,
	uBegin,
	uEnd,
	...
	var loc_ignoreRoute =
		typeof ignoreRoute === "undefined" ? false : ignoreRoute; // default: routes  matter at diverges
	if (isMerge) loc_ignoreRoute = true; // merging must be always possible

	var loc_prioOther = typeof prioOther === "undefined" ? false : prioOther;

	var loc_prioOwn = typeof prioOwn === "undefined" ? false : prioOwn;

	if (loc_prioOwn && loc_prioOther) {
		console.log(
			"road.mergeDiverge: Warning: prioOther and prioOwn" +
	...
	if (success) {
		iMerge = 0;
		uTarget = originVehicles[0].u + offset;
	}
	if (
		success ||
		(originVehicles.length > 0 &&
	...
	// (2b) otherwise select the first suitable candidate of originVehicles
		...
		if (log) {
			console.log(" entering origVeh loop");
		}
		...
		for (var i = 0; i < originVehicles.length && !success; i++) {
			...
			if (
				...
				var vrel =
					originVehicles[i].speed / originVehicles[i].longModel.v0;

				var acc = originVehicles[i].acc;
				var accNew = originVehicles[i].longModel.calcAccDet(
					//MOBIL
					sNew,
					speed,
					speedLeadNew,
				...
				var routeOK = (MOBILOK = LCModel.realizeLaneChange(
					vrel,
					acc,
					accNew,
					accLagNew,
	...
	if (isMerge && loc_prioOwn) {
		...
		for (var j = 0; j < targetVehicles.length; j++) {
			...
			var accTargetStop = targetVehicles[j].longModel.calcAccDet(
				sStop,
				speedTarget,
				0,
				0
			...
			if (iLast > -1 && targetVehicles[j].isRegularVeh()) {
				...
				if (lastOrigIsLeader) {
					...
					var accLTC = targetVehicles[j].longModel.calcAccDet(
						s,
						speedTarget,
						speedOrig,
						0
					);
					var accTarget = Math.min(
						targetVehicles[j].acc,
						Math.max(accLTC, accTargetStop)
					);
					if (accTarget > -bSafe) {
						targetVehicles[j].acc = accTarget;
						if (this.markVehsMerge) {
							targetVehicles[j].colorStyle = 3;
						}
					}
				}
	...
	if (success) {
		...
		if (true) {
			console.log(
				"\n=========================================\n",
				"mergeDiverge (4) Actual merging vehicle id " +
					this.veh[iOrig].id +
...
road.prototype.updateBCup = function (Qin, dt, route) {
	...
	if (emptyOverfullBuffer && this.inVehBuffer > 2) {
		this.inVehBuffer--;
	}
	...
	if (this.inVehBuffer >= this.randomValBCup) {
		...
		if (!success && vehType === "truck") {
			...
			while (iLead > 0 && this.veh[iLead].lane != lane) {
				iLead--;
			}
			if (iLead == -1) {
				success = true;
		...
		if (success) {
			var longModelNew =
				vehType === "truck" ? longModelTruck : longModelCar;
			...
			var speedNew = Math.min(
				v0New,
				longModelNew.speedlimit,
...
road.prototype.getTargetNeighbourhood = function (umin, umax, targetLane) {
	...
	if (false) {
		console.log(
			"in road.getTargetNeighbourhood(umin=" +
				umin +
				"+ umax=" +
				umax +
				"+ targetLane=" +
		...
		for (iTarget = 0; iTarget < targetVehicles.length; iTarget++) {
			console.log(
				"targetVehicles[" + iTarget + "].u=" + targetVehicles[iTarget].u
			);
		}
	}
...
road.prototype.updateModelsOfAllVehicles = function (
	longModelCar,
	longModelTruck,
	LCModelCar,
	LCModelTruck,
	...
	this.longModelTacticalCar = new ACC(
		longModelCar.v0,
		longModelCar.T,
		longModelCar.s0,
		longModelCar.a,
	...
	this.longModelTacticalTruck = new ACC(
		longModelTruck.v0,
		longModelTruck.T,
		longModelTruck.s0,
		longModelTruck.a,
	...
	if (testNewModel) {
		this.longModelTacticalCar = new CACC(
			longModelCar.v0,
			longModelCar.T,
			longModelCar.s0,
			longModelCar.a,
		...
		this.longModelTacticalTruck = new CACC(
			longModelTruck.v0,
			longModelTruck.T,
			longModelTruck.s0,
			longModelTruck.a,
	...
	var LCModelMandatoryLoc = new MOBIL(
		LCModelMandatory.bSafe,
		LCModelMandatory.bSafeMax,
		LCModelMandatory.p,
		LCModelMandatory.bThr,
		LCModelMandatory.bBiasRight
	);

	this.LCModelMandatoryRight = new MOBIL(
		LCModelMandatoryLoc.bSafe,
		LCModelMandatoryLoc.bSafeMax,
		LCModelMandatoryLoc.p,
		LCModelMandatoryLoc.bThr,
		LCModelMandatoryLoc.bBiasRight
	);
	this.LCModelMandatoryLeft = new MOBIL(
		LCModelMandatoryLoc.bSafe,
		LCModelMandatoryLoc.bSafeMax,
		LCModelMandatoryLoc.p,
		LCModelMandatoryLoc.bThr,
	...
	this.LCModelTacticalRight = new MOBIL(
		LCModelMandatoryLoc.bSafe,
		LCModelMandatoryLoc.bSafeMax,
		LCModelMandatoryLoc.p,
		LCModelMandatoryLoc.bThr,
	...
	this.LCModelTacticalLeft = new MOBIL(
		LCModelMandatoryLoc.bSafe,
		LCModelMandatoryLoc.bSafeMax,
		LCModelMandatoryLoc.p,
		LCModelMandatoryLoc.bThr,
	...
	for (var i = 0; i < this.veh.length; i++) {
		if (this.veh[i].isRegularVeh()) {
			this.veh[i].longModel.copy(
				this.veh[i].type === "truck" ? longModelTruck : longModelCar
			);
			this.veh[i].LCModel.copy(
				this.veh[i].type === "truck" ? LCModelTruck : LCModelCar
			);
			...
			if (false) {
				console.log(
					"updateModelsOfAllVehicles: type=" + this.veh[i].type,
	...
	for (var ir = 0; ir < this.mergeDivergeInfo.length; ir++) {
		...
		for (var i = 0; i < this.veh.length; i++) {
			if (
				...
				if (
					this.veh[i].u < uLast &&
					this.veh[i].u > uLast - this.duTactical
				) {
					...
					if (!isMerge) {
						...
						this.veh[i].longModel.alpha_v0 = Math.max(
							0.5,
							(uLast - this.veh[i].u) / this.duTactical
						);
					}
					...
					if (false) {
						console.log(
							"road.updateModelsOfAllVehicles: apply tacticalLC to Veh" +
								...
								toRight +
								" u=" +
								parseFloat(this.veh[i].u).toFixed(1) +
								" uLast=" +
								parseFloat(uLast).toFixed(1) +
								" bBiasRight=" +
								this.veh[i].LCModel.bBiasRight
						);
					}
				}
			} // nextOfframpNearby
			else {
				// also for missed diverges
...
road.prototype.get_phi = function (u, traj) {
	...
	if (dx < 0 || (Math.abs(dx) < smallVal && dy < 0)) {
		phi += Math.PI;
	}
...
road.prototype.draw = function (
	...
	var movingObserver = typeof movingObs === "undefined" ? false : movingObs;
	...
	var nSegmLine =
		2 * Math.round((0.5 * duLine) / (this.roadLen / this.nSegm)); // 0,2,4...
	...
	var wSegmPix =
		scale * (this.nLanes * this.laneWidth + this.boundaryStripWidth);
	...
	for (var iSegm = 0; iSegm < this.nSegm; iSegm++) {
		...
		if (filterPassed) {
			...
			var xCenterPix =
				scale * (this.draw_x[iSegm] - this.traj[0](uRef) + xRef);
			var yCenterPix =
				-scale * (this.draw_y[iSegm] - this.traj[1](uRef) + yRef);
			...
			var roadImg =
				iSegm % nSegmLine < nSegmLine / 2 ? roadImg1 : roadImg2;
			ctx.drawImage(
				roadImg,
				-0.5 * lSegmPix,
				-0.5 * wSegmPix,
			...
			if (false) {
				...
				console.log(
					"road.draw: ID=" + this.roadID,
					" iSegm=" + iSegm,
					" this.draw_y[iSegm]=" + formd(this.draw_y[iSegm]),
					" this.traj[1](this.roadLen-50)=" +
						formd(this.traj[1](this.roadLen - 50)),
					" lSegmPix=" + formd(lSegmPix),
					" wSegmPix=" + formd(wSegmPix),
...
*/
	...
	if (!movingObserver && this.drawAlternativeTrajectories) {
		for (var iTraj = 0; iTraj < this.trajAlt.length; iTraj++) {
			var nSegmAlt =
				(this.trajAlt[iTraj].umax - this.trajAlt[iTraj].umin) / lSegm;
			var trajAlt = [this.trajAlt[iTraj].x, this.trajAlt[iTraj].y];
			var laneMin =
				typeof this.trajAlt[iTraj].laneMin === undefined
					? 0
					: this.trajAlt[iTraj].laneMin;
			var laneMax =
				typeof this.trajAlt[iTraj].laneMax === undefined
					? this.nLanes
					: this.trajAlt[iTraj].laneMax;

			var wSegmPix =
				scale * (laneMax - laneMin + 1) * this.laneWidth +
				this.boundaryStripWidth;

			var vCenterPhys =
				this.laneWidth * 0.5 * (laneMin + laneMax + 1 - this.nLanes);
...
road.prototype.drawTaperRamp = function (roadImg1, laneIncr, atRight) {
	...
	if (laneIncr > 0) {
		vStart -= laneIncr * (atRight ? 1 : -1) * this.laneWidth;
	}
...
road.prototype.drawTaperConnect = function (roadImg1, targetID) {
	...
	for (var i = 0; i < this.connectInfo.length; i++) {
		if (this.connectInfo[i].targetID == targetID) {
			ir = i;
		}
	}
	...
	if (ir > -1) {
		...
		var incrRight =
			-incrLeft + this.connectInfo[ir].targetNlanes - this.nLanes;
...
road.prototype.drawTaper = function (roadImg1, laneShift, uStart, vStart) {
	...
	for (var iSegm = 0; iSegm < nSegmTaper; iSegm++) {
		...
		var u = uStart + du - 0.1 * this.taperLen; //!! to make taper look shorter
		...
		if (debug) {
			//if(false){
			console.log(
				"road.drawTaper: roadID=" + this.roadID,
				" iSegm=" + iSegm,
				...
				"xCenterPix=",
				xCenterPix.toFixed(0),
				"yCenterPix=",
				yCenterPix.toFixed(0),
				""
			);
		}
	}
...
road.prototype.drawRoadID = function () {
	...
	ctx.fillText("road " + this.roadID, -0.45 * lPix, 0.2 * hPix);
};
...
road.prototype.drawVehicles = function (
	...
	if (false) {
		console.log(
			"road.drawVehicles: traj init:" + " this.traj[0]=" + this.traj[0]
		);
	}
	...
	for (var i = 0; i < this.veh.length; i++) {
		...
		var filterPassed =
			(!this.veh[i].isTrafficLight() || this.drawVehIDs) &&
			(noRestriction || // default: noRestriction=true
...
road.prototype.drawVehicle = function (
	i,
	carImg,
	truckImg,
	obstacleImg,
	...
	var vCenterPhys =
		this.laneWidth * (this.veh[i].v - 0.5 * (this.nLanes - 1));

	// (2) determine vehicle orientation

	var phiRoad = this.get_phi(uCenterPhys, traj);
	var phiVehRel =
		this.veh[i].speed < 0.1
			? 0
			: -Math.atan(
					(this.veh[i].dvdt * this.laneWidth) / this.veh[i].speed
			  );

	phiVehRel = Math.max(-phiVehRelMax, Math.min(phiVehRelMax, phiVehRel));
	...
	var obstacleImgIndex = this.veh[i].isSpecialVeh()
		? (this.veh[i].id - 49) % obstacleImgs.length
		: 0;
	...
	if (type === "obstacle" || upright == true) {
		...
		if (obstacleImgIndex != 0) {
			// index 0: black bar for ramp ends, OK
			phiVeh -= 0.2;
	...
	var xCenterPix =
		scale * (traj[0](uCenterPhys) + vCenterPhys * sphiRoad - xOffset); // -xOffset=-trajLoc[0](uRef)+xRef)
	var yCenterPix =
		-scale * (traj[1](uCenterPhys) - vCenterPhys * cphiRoad - yOffset);
	...
	if (type != "obstacle" && speedmax > 1e-10) {
		// no box if speedmin=speedmax=0
		var effLenPix = type === "truck" ? 0.9 * vehLenPix : 1.0 * vehLenPix;
		var effWPix = type === "truck" ? 0.8 * vehWidthPix : 0.6 * vehWidthPix;
		var speed = this.veh[i].speed;
		var isEgo = this.veh[i].id === 1;
		ctx.fillStyle =
			this.veh[i].colorStyle == 0
				? colormapSpeed(speed, speedmin, speedmax, type, isEgo, time)
				: this.veh[i].colorStyle == 1
				? "rgba(0,0,255,0.5)"
				: this.veh[i].colorStyle == 2
				? "rgba(255,255,0,0.5)"
				: "rgba(255,0,0,0.5)";
	...
	if (this.drawVehIDs) {
		...
		ctx.fillRect(-0.5 * lPix, -0.5 * hPix, lPix, hPix);
		ctx.fillStyle = "rgb(0,0,0)";
		ctx.fillText(this.veh[i].id, -0.45 * lPix, 0.4 * hPix);
	}
	...
	if (false) {
		//if(this.veh[i].v>2){
		console.log(
			"in road.drawVehicle: itime=" + itime,
			+" u=" +
				this.veh[i].u +
				" v=" +
				this.veh[i].v +
...
road.prototype.updateSpeedlimits = function (trafficObjects) {
	...
	for (var i = 0; i < trafficObjects.trafficObj.length; i++) {
		...
		if (
			...
			if (false) {
				console.log(
					"road.updateSpeedlimits: speed limit " + formd(speedL),
					" starting at " + formd(obj.u)
				);
			}
			...
			for (var iveh = 0; iveh < this.veh.length; iveh++) {
				...
				var neededDecel =
					sLimit < 0
						? 0
						: (0.5 * (speed + speedL) * (speed - speedL)) / sLimit;
				if (sLimit < duAntic || neededDecel > bBrake) {
					//if(sLimit<duAntic){
					if (targetVeh.isRegularVeh()) {
						targetVeh.longModel.speedlimit =
							targetVeh.type === "truck"
								? Math.min(speedL, speedL_truck)
								: speedL;
					}
				}
			}
...
road.prototype.dropObject = function (trafficObj) {
	...
	if (trafficObj.type === "obstacle") {
		...
		console.log(
			"  end road.dropObject: dropped obstacle at uDrop=" + u,
			" lane=" + lane,
			" id=" + roadVehicle.id,
...
road.prototype.addTrafficLight = function (depotObject) {
	...
	if (true) {
		console.log(
			"itime=" + itime,
			" road.addTrafficLight: roadID=" + this.roadID,
			" added traffic light id=" + depotObject.id,
			" at u=" + formd(depotObject.u),
			" value=" + depotObject.value
		);
	}
};
...
road.prototype.changeTrafficLight = function (id, value) {
	...
	for (var i = 0; !success && i < this.trafficLights.length; i++) {
		if (id === this.trafficLights[i].id) {
			...
			if (typeof value === "undefined") {
				// just toggle if no value given
				pickedTL.value = pickedTL.value === "red" ? "green" : "red";
				console.log(
					"road.changeTrafficLight: id=" + id,
					"no TL state given:" +
						" new value=opposite of old value=" +
						pickedTL.value
				);
			} else {
				pickedTL.value = value;
			}
		}
	}
	...
	// commands changeTrafficLight(id,"red") without (id,"green") given!
		...
		for (var i = 0; i < this.veh.length; i++) {
			if (this.veh[i].id === id) {
				virtVehAlreadyExist = true;
			}
		}

		if (!virtVehAlreadyExist) {
			for (var il = 0; il < this.nLanes; il++) {
				var virtVeh = new vehicle(
					1,
					this.laneWidth,
					pickedTL.u,
					il,
...
road.prototype.removeTrafficLight = function (id) {
	...
	console.log(
		"in road.removeTrafficLight: id=" + id,
	...
	for (var i = 0; !success && i < this.trafficLights.length; i++) {
		if (this.trafficLights[i].id === id) {
			...
			console.log(
				"  succes! i=" + i,
				" trafficLight=" + this.trafficLights[i]
			);
			iDel = i;
	...
	if (iDel === -1)
		console.log("road.removeTrafficLight: no id " + id, " found!");
