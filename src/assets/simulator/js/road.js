...
function road(
  ...
  this.isGame =
    scenarioString === "RampMeteringGame" || scenarioString === "RoutingGame";
...
road.prototype.getAttributes = function (fracTruck, fracOthers) {
  ...
  var r = Math.random();
  var vehType =
    r < fracTruck ? "truck" : r < fracTruck + others_frac ? "others" : "car";
  var lengthOut =
    vehType === "car"
      ? car_length
      : vehType === "truck"
...
road.prototype.updateExportString = function () {
  ...
  if (rest < dt - 0.0001) {
    for (var i = 0; i < this.veh.length; i++) {
      var heading =
        this.veh[i].speed > 1e-4 ? this.veh[i].dvdt / this.veh[i].speed : 0;
      this.exportString =
        this.exportString +
        "\n" +
        time.toFixed(2) +
...
road.prototype.findNearestDistanceTo = function (xUser, yUser) {
  ...
  var sign_v =
    Math.cos(phiNorm) * dxReturn + Math.sin(phiNorm) * dyReturn > 0 ? 1 : -1;
...
road.prototype.initializeMicro = function (
  ...
  for (var i = 0; i < types.length; i++) {
    ...
    var type = types[i] === 0 ? "car" : types[i] === 1 ? "truck" : "obstacle";
  ...
  // too few vehicles, generate one per time step in largest gap
    ...
    if (success) {
      ...
      if (!emptyLanes) {
        ...
        speedNew =
          0.5 * (this.veh[k].speed + this.veh[this.veh[k].iLead].speed);
      }
...
road.prototype.update_iLeadRight = function (i) {
  ...
  if (this.veh[i].lane < this.nLanes - 1) {
    ...
    success =
      i === iLeadRight || this.veh[iLeadRight].lane === this.veh[i].lane + 1;
    while (!success) {
...
road.prototype.update_iLagRight = function (i) {
  ...
  if (this.veh[i].lane < this.nLanes - 1) {
    ...
    success =
      i === iLagRight || this.veh[iLagRight].lane === this.veh[i].lane + 1;
    while (!success) {
...
road.prototype.update_iLeadLeft = function (i) {
  ...
  if (this.veh[i].lane > 0) {
    ...
    success =
      i === iLeadLeft || this.veh[iLeadLeft].lane === this.veh[i].lane - 1;
    while (!success) {
...
road.prototype.updateEgoVeh = function (externalEgoVeh) {
  ...
  var acc_v =
    (externalEgoVeh.aLat + roadCurv * ego.speed * ego.speed) / this.laneWidth; // [lanes/s^2]
    ...
    if (
      ...
      if (targetLaneExists && lastChangeSufficTimeAgo) {
        ...
        if (
          ...
          var sNew =
            this.veh[iLeadNew].u - this.veh[iLeadNew].len - this.veh[i].u;
        ...
        changeSuccessful =
          this.veh[i].isRegularVeh() && sNew > 0 && sLagNew > 0 && MOBILOK;
        ...
        if ((C2 || C3) && !gluedTogether) {
          ...
          if (targetFlwExists && speed < speedTargetFlw) {
            ...
            var tc =
              -speed / accFree +
              Math.sqrt(
                Math.pow(speed / accFree, 2) + (2 * (uSource - u)) / accFree
              );
          ...
          if (potentialConflictsExist) {
            ...
            if (
              uTargetLdr - lenTargetLdr - vehConnect.len - 2 * s0 <
              ducExitMax
            ) {
          ...
          vehConnect.finalGo =
            vehConnect.finalGo ||
            (C3 && vehConnect.tookGoDecision && !vehConnect.conflictsExist);
 	...
 	###############################################################*/
        ...
        if (!gluedTogether) {
          if (C1) {
            ...
            //else if(C2||C3){
            vehConnect.acc = vehConnect.tookGoDecision ? accConnect : accStop;
          }
        }
      ...
      if (u > uSource && this.roadID != targetRoad.roadID) {
        ...
        if (false) {
          console.log(
            "\nbefore splicing:" + " this.veh.length=" + this.veh.length,
            " testVeh=" + testVeh,
...
road.prototype.determineConflicts = function (
  ...
  for (var ic = 0; ic < conflicts.length && noConflictDetected; ic++) {
    ...
    var tc =
      -speed / acc + Math.sqrt(Math.pow(speed / acc, 2) + (2 * duOwn) / acc);
    ...
    for (var iveh = 0; goOnCrit; iveh++) {
      ...
      if (vehConflict.isRegularVeh()) {
        ...
        if (conflicts[ic].dest.length == 0) {
          ...
          if (log) {
            console.log(
              "  in conflicting veh loop: vehConflict.id=" + vehConflict.id,
      ...
      goOnCrit =
        iveh < vehsConflict.length - 1 && xtc < smax && noConflictDetected;
...
road.prototype.mergeDiverge = function (
  ...
  var success =
    ...
    (loc_ignoreRoute || originVehicles[0].route.includes(otherRoad.roadID)) &&
    ...
    for (var i = 0; i < originVehicles.length && !success; i++) {
      if (log) {
        console.log(
          ...
          " originVehicles[i].divergeAhead=" + originVehicles[i].divergeAhead
        );
      }
...
road.prototype.updateModelsOfAllVehicles = function (
  ...
  for (var ir = 0; ir < this.mergeDivergeInfo.length; ir++) {
    ...
    for (var i = 0; i < this.veh.length; i++) {
      if (this.veh[i].isRegularVeh() && this.veh[i].route.includes(targetID)) {
...
road.prototype.drawTaperConnect = function (roadImg1, targetID) {
  ...
  if (ir > -1) {
    ...
    var incrRight = -incrLeft + this.connectInfo[ir].targetNlanes - this.nLanes;
  ...
  for (var i = 0; i < trafficObjects.trafficObj.length; i++) {
    ...
    if (
      ...
      for (var iveh = 0; iveh < this.veh.length; iveh++) {
        ...
        var neededDecel =
          sLimit < 0 ? 0 : (0.5 * (speed + speedL) * (speed - speedL)) / sLimit;
...
/**
 * Verifica se parte da array B está em A
 *
 * @param {*} a Array A
 * @param {*} b Array B
 * @returns {boolean}
