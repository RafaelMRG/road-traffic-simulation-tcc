...
function toggleRoadID() {
	if (drawRoadIDs) {
		...
		drawRoadIDs = true;
		document.getElementById("buttonRoadID").innerHTML =
			"Do not show road IDs";
	}
}
...
function toggleVehID() {
	if (drawVehIDs) {
		...
		document.getElementById("buttonVehID").innerHTML =
			"Do not show vehicle IDs";
	}
}
...
function showHideTLeditPanel() {
	if (trafficLightControl.isActive) {
		...
		if (drawBackground) {
			// wipe out existing editor panel
...
function handleIntersectionType(index) {
	...
	console.log(
		"in handleIntersectionType: index=",
		index,
		" intersectionType=",
		intersectionType[index]
	);
}
...
function handleChangedPriority(index) {
	...
	console.log(
		"in handleChangedPriority: index=",
		index,
		" respectRingPrio=",
		respectRingPrio
	);
...
function updateHighscores(nickName, newScore, storageName) {
	...
	scores.push({ name: nickName, score: newScore, date: dateStr });
	...
	var str_highScores =
		"<h1> Game Finished!</h1> Your time is " +
		newScore +
		" Seconds" +
...
function showInfo() {
	...
	if (infoLevel === 0) {
		$("#infotext").load("info/info_gui.html");
	} else if (infoLevel === 1) {
		$("#infotext").load(scenarioFile);
	} else if (infoLevel === 2) {
		$("#infotext").load("info/info_IDM.html");
	} else if (infoLevel === 3) {
		$("#infotext").load("info/info_MOBIL.html");
	} else if (infoLevel === 4) {
		$("#infotext").load("info/info_BC.html");
	} else if (infoLevel === 5) {
		$("#infotext").load("info/info_Numerics.html");
	}
	infoLevel++;
...
function addOneLane() {
	if (mainroad.nLanes < nLanesMax) {
		...
		console.log("addOneLane(): maximum of ", nLanesMax, " lanes reached!");
	}
...
function toggleTruckOvertakingBan() {
	...
	if (false) {
		console.log(
			"control_gui.toggleTruckOvertakingBan: banIsActive=",
			banIsActive,
			" LCModelMandatory=",
			LCModelMandatory,
			" LCModelTruckUphill=",
...
var leftTurnBias = (leftTurnBias = 0.1); // 0.10
...
if (document.getElementById("slider_MOBIL_bBiasRight_car") !== null) {
	slider_MOBIL_bBiasRight_car = document.getElementById(
		"slider_MOBIL_bBiasRight_car"
	);
	slider_MOBIL_bBiasRight_carVal = document.getElementById(
...
if (document.getElementById("slider_MOBIL_bBiasRight_truck") !== null) {
	slider_MOBIL_bBiasRight_truck = document.getElementById(
		"slider_MOBIL_bBiasRight_truck"
	);
	slider_MOBIL_bBiasRight_truckVal = document.getElementById(
...
if (document.getElementById("slider_MOBIL_p") !== null) {
	slider_MOBIL_p = document.getElementById("slider_MOBIL_p");
	slider_MOBIL_pVal = document.getElementById("slider_MOBIL_pVal");
	slider_MOBIL_p.value = MOBIL_p;
	slider_MOBIL_pVal.innerHTML = MOBIL_p + " m/s<sup>2</sup>";
...
function updateModels() {
	...
	LCModelCar = new MOBIL(
		MOBIL_bSafe,
		MOBIL_bSafeMax,
		MOBIL_p,
		MOBIL_bThr,
		MOBIL_bBiasRight_car
	);
...
function updateModelsUphill() {
	...
	if (false) {
		console.log(
			"control_gui.updateModelsUphill: banIsActive=",
			banIsActive,
			"LCModelMandatory=",
			LCModelMandatory,
			" LCModelTruckUphill=",
...
function downloadCallback() {
	if (downloadActive) {
		...
		document.getElementById("download").src =
			"figs/iconDownloadStart_small.png";
	} else {
		// title/header lines
		...
		downloadActive = true;
		document.getElementById("download").src =
			"figs/iconDownloadFinish_small.png";
	}
}

function performDownload() {
	// callback of download finish button
	var msg = "";
	for (var i = 0; i < network.length; i++) {
		var filename =
			"road" + network[i].roadID + "_time" + time.toFixed(1) + ".txt";
		msg = msg + filename + " ";
	...
	if (typeof detectors !== "undefined") {
		for (var iDet = 0; iDet < detectors.length; iDet++) {
			var filename =
				...
				iDet +
				"_road" +
				detectors[iDet].road.roadID +
				"_x" +
				detectors[iDet].u.toFixed(0) +
				"_time" +
				time.toFixed(0) +
				".txt";
			msg = msg + filename + " ";
