...
function stationaryDetector(road, u, dtAggr) {
	...
	this.i = detectorIndex;
	detectorIndex++;

	if (this.u > road.roadLen) {
		console.log(
			"Warning: trying to place a detector at position u=",
			this.u,
			" greater than the road segment length ",
			road.roadLen,
			" resetting u=",
			road.roadLen
		);
...
stationaryDetector.prototype.update = function (time, dt) {
	...
	if (vehNear.id != this.vehNearOld.id) {
		...
		if (false) {
			console.log(
				"stationaryDetector.update: new single-veh",
				" t=",
				time,
				" lane=",
				i,
...
stationaryDetector.prototype.display = function (textsize) {
	...
	var flowStr =
		"Flow: " + Math.round(3600 * this.historyFlow[this.iAggr]) + " veh/h";
	var speedStr =
		"Speed: " +
		(this.historyFlow[this.iAggr] > 0
			? Math.round(3.6 * this.historySpeed[this.iAggr])
	...
	var densStr =
		...
		(this.historyFlow[this.iAggr] > 0
			? Math.round(
					(1000 * this.historyFlow[this.iAggr]) /
						this.historySpeed[this.iAggr]
			  )
			: "--") +
		" veh/km";
	...
	var toRight_axis =
		-0.5 * roadWidth - (2.2 + 1.8 * Math.abs(sphi)) * laneWidth;
